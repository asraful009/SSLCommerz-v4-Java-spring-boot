package com.cyber009.sslcommerz.sslutility;
...
import com.cyber009.sslcommerz.sslutility.parametermappings.SSLCommerzInitResponse;
import com.cyber009.sslcommerz.sslutility.parametermappings.SSLCommerzValidatorResponse;

import lombok.extern.slf4j.Slf4j;

import com.cyber009.sslcommerz.sslutility.Utility.*;;

/**
 * This class is responsible for initiating Transaction request
 * Generating request URL with Session or JSON list to perform payment option
 * selection
...
public class SSLCommerz {
     ...
     * @param Store_Pass
     *                        Store_Pass is that password of user to connect
     *                        SSLCOmmerz
     * @param Store_Test_Mode
     *                        Store_Test_Mode is to define whether user is testing
     *                        this or not.
     * @throws Exception
     */
     ...
     *
     * @param storeTestMode
     *                      storeTestMode defines user Testing mode and sets test
     *                      id, password and sandbox URL
     */
     ...
     *                         Construct this Map with Necessary Key value pair to
     *                         provide request parameters.
     * @param isGetGatewayList
     *                         isGetGatewayList is to define whether user wants to
     *                         get response as Gateway list or not
     * @return
     *         Returns a String which is an URL with Session Key to perform payment
     *         option selection,
    ...
    public String initiateTransaction(Map<String, String> postData, boolean isGetGatewayList) throws Exception {
        ...
        try {
            ...
            if (resp.status.equals("SUCCESS")) {
                ...
                if (isGetGatewayList) {
                    // We will work on it!
                    return String.format("{\"status\": \"success\", \"data\": \"%s\", \"logo\": \"%s\" }",
                            resp.getGatewayPageURL(),
                            resp.getStoreLogo());
                } else {
     ...
     * @param postData
     *                 postData is the request parameter Map comprised of key value
     *                 pair
     * @return
     *         Returns a String which is an URL with Session Key to perform payment
     *         option selection,
     ...
     *                              merchantTrnxnAmount: merchants transaction
     *                              amount.
     * @param merchantTrnxnCurrency
     *                              merchantTrnxnCurrency: Merchants transaction
     *                              amount
     * @param requestParameters
     *                              requestParameters is a Map of String as key and
     *                              String as value
     *                              which should be constructed from the success
     *                              response of the payment page.
     * @return
    ...
    boolean orderValidate(String merchantTrnxnId, String merchantTrnxnAmount, String merchantTrnxnCurrency,
        ...
        if (hash_verified) {
            ...
            if (!json.isEmpty()) {
                SSLCommerzValidatorResponse resp = Util.extractValidatorResponse(json);// new
                                                                                       // JavaScriptSerializer().Deserialize
                                                                                       // < SSLCommerzValidatorResponse
                                                                                       // > (json);
     ...
     *
     * @param requestParameters
     *                          requestParameters is a Map of String which should be
     *                          constructed from the RequestParameter returned from
     *                          the success response of payment page.
     * @return
     *         Returns if the Hash generated during SSL Commerz payment request and
     *         the hash generated from the
